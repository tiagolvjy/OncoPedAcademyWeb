...
export default function QuestionForm({ questionnaireID }: { questionnaireID: string }) {
    ...
    const handleSubmit = async () => {
        ...
        if (!order || Number(order) < 1) {
            setError('Digite a ordem da questão.');
            return;
        }
        ...
        const { success, error } = await QuestionnaireServices.createQuestion(questionnaireID, {
            text,
            options,
            order: Number(order),
        ...
        if (success) {
            setFlashData({ success: 'Questão criada com sucesso!' });
            router.replace(`/admin/questionarios/${questionnaireID}`);
        } else {
            setError(error ?? 'Erro ao criar questão.');
        }
    }
    // ===========================================================================
    return (
        <div className="flex flex-col gap-4">
            ...
            <div>
                <p className="text-sm font-medium mb-2">
                    Imagem da questão: <span className="text-gray-400 font-normal">(opcional)</span>
                </p>
                <input
                    ref={fileInputRef}
                    type="file"
                ...
                {imageFileName && !imageURL && (
                    <p className="text-sm text-gray-500 mt-2">{imageFileName}</p>
                )}
                {uploading && (
                    <div className="mt-2">
                ...
                {imageURL && (
                    <img src={imageURL} alt="Imagem da questão" className="mt-2 max-w-[300px] rounded" />
                )}
            </div>

            {/* ALTERNATIVAS */}
            <div>
                <p className="text-sm font-medium mb-3">Alternativas: <span className="text-gray-400 font-normal">(marque a correta)</span></p>
                        ...
                        <div key={option.id} className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-bold text-sm flex-shrink-0 transition-all ${
                                    option.isCorrect
                                        ? 'bg-[#1aab67] border-[#1aab67] text-white'
                ...
                {options.length < 6 && (
                    <button
                        ...
                        className="mt-3 text-sm text-[#4703D0] hover:opacity-70 flex items-center gap-1"
                    >
